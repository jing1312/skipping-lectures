...
### 4.5 失败即改道（内嵌 fallback）

| 失败现象 | 直接改道 |
|---|---|
| 抽音频/下载 401/403 | 直链过期：重跑 `rebuild_fresh_manifest.cjs` 再导 CSV，**不重采课程列表** |
| CDP 连不上 | 检查 9222 端口与调试浏览器是否保持打开；不换端口硬试 |
| ASR 失败/无结果 | 查 `X-Api-Status-Code` 与 `volc.seedasr.auc` 资源 ID；不行换 MiMo 备选 |
| 产物是 85~91 字占位 | 服务端生成失败：重跑无用，网页端手动重试该节，然后跳过继续 |
| 页面改版脚本失效 | 按接口要点重新探测（CDP 看 Vue 组件、抓接口），不硬猜 |

所有断点续传：PPT 看 `state.json`，讲稿/笔记按本地文件存在跳过，`--force` 重跑。
